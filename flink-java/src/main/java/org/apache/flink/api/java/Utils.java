...
public final class Utils {
	...
	private static String getGenericTypeTree(Class<?> type, int indent) {
		...
		for(Field field : type.getDeclaredFields()) {
			...
			ret += StringUtils.repeat(' ', indent) + field.getName() + ":" + field.getType().getName() + (field.getType()
				.isEnum() ? " (is enum)" : "") + "\n";
	...
	public static String getKeyFromArgs(String[] args, int index) {
		...
		if(args[index].startsWith("--")) {
			...
			throw new IllegalArgumentException(
				String.format("Error parsing arguments '%s' on '%s'. Please prefix keys with -- or -.", Arrays.toString(args), args[index]));
		}
