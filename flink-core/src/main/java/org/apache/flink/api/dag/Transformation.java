...
/**
 * // TODO_MA 注释：{@code Transformation}表示创建DataStream的操作。
 * // TODO_MA 注释：每个DataStream都有一个基础的{@code Transformation}，它是该DataStream的起源。
 ...
 *
 * // TODO_MA 注释：API操作（例如DataStream＃map）在下面创建了{@code Transformation} s的树。
 * // TODO_MA 注释：当要执行流程序时，使用StreamGraphGenerator将该图转换为StreamGraph。
 ...
 *
 * // TODO_MA 注释：{@ code Transformation}不一定与运行时的物理操作相对应。 某些操作仅是逻辑概念。例如联合，拆分/选择数据流，分区。
