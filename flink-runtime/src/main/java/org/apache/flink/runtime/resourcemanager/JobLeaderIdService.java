...
public class JobLeaderIdService {
	...
	public JobLeaderIdService(HighAvailabilityServices highAvailabilityServices, ScheduledExecutor scheduledExecutor, Time jobTimeout) {
	...
	public void clear() throws Exception {
		...
		if(exception != null) {
			ExceptionUtils.rethrowException(exception, "Could not properly stop the " + JobLeaderIdService.class.getSimpleName() + '.');
		}
	...
	private final class JobLeaderIdListener implements LeaderRetrievalListener {
		...
		private JobLeaderIdListener(JobID jobId, JobLeaderIdActions listenerJobLeaderIdActions,
		...
		public void notifyLeaderAddress(String leaderAddress, UUID leaderSessionId) {
			if(running) {
				...
				LOG.debug("A leader id change {}@{} has been detected after the listener has been stopped.", leaderSessionId, leaderAddress);
			}
		}
		...
		public void handleError(Exception exception) {
			if(running) {
				...
				LOG.debug("An error occurred in the {} after the listener has been stopped.", JobLeaderIdListener.class.getSimpleName(), exception);
			}
		}
