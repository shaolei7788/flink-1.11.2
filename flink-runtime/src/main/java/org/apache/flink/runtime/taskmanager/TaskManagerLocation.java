...
public class TaskManagerLocation implements Comparable<TaskManagerLocation>, java.io.Serializable {
	...
	public static TaskManagerLocation fromUnresolvedLocation(final UnresolvedTaskManagerLocation unresolvedLocation) throws UnknownHostException {
		return new TaskManagerLocation(unresolvedLocation.getResourceID(), InetAddress.getByName(unresolvedLocation.getExternalAddress()),
	...
	private static String getFqdnHostName(InetAddress inetAddress) {
		...
		try {
			...
			LOG.warn(
				"Unable to determine the canonical hostname. Input split assignment (such as " + "for HDFS files) may be non-local when the canonical hostname is missing.");
	...
	public static String getHostName(InetAddress inetAddress) {
		...
		if(fqdnHostName.equals(inetAddress.getHostAddress())) {
			...
			LOG.warn(
				"No hostname could be resolved for the IP address {}, using IP address as host name. " + "Local input split assignment (such as for HDFS files) may be impacted.",
