...
public class HighAvailabilityServicesUtils {
	...
	public static HighAvailabilityServices createHighAvailabilityServices(Configuration configuration, Executor executor,
		...
		// TODO_MA 注释：获取 HA 模式, 一般，我们都是设置 zookeeper 模式
		// TODO_MA 注释： 在 fink-conf.yaml 配置文件中，我们会去配置：high-availability = zookeeper
		...
		switch(highAvailabilityMode) {
			...
			case NONE:
				...
				// TODO_MA 注释： 如果不是高可用：StandaloneHaServices
			...
			case ZOOKEEPER:
				...
				// TODO_MA 注释： 获取 BlobStoreService
				BlobStoreService blobStoreService = BlobUtils.createBlobStoreFromConfig(configuration);

				// TODO_MA 注释： 创建 ZooKeeperHaServices
				// TODO_MA 注释： 对象的内部，包装了一个 ZooKeeper 的实例对象
				// TODO_MA 注释： 内部的 zk 代码实现： 用的 curator
	...
	public static Path getClusterHighAvailableStoragePath(Configuration configuration) {
		...
		// TODO_MA 注释： 获取 HA 模式中，存储在 HDFS 上的数据的路径
		// TODO_MA 注释： high-availability.storageDir
		...
		// TODO_MA 注释： path 封装 storagePath
		final Path path;
		try {
		...
		// TODO_MA 注释： clusterStoragePath 封装 path
