...
public class MemoryLogger extends Thread {
	...
	public static void startIfConfigured(Logger logger, Configuration configuration, CompletableFuture<Void> taskManagerTerminationFuture) {
		...
		new MemoryLogger(logger, configuration.getLong(TaskManagerOptions.DEBUG_MEMORY_USAGE_LOG_INTERVAL_MS), taskManagerTerminationFuture).start();
	}
	...
	public static String getMemoryUsageStatsAsString(MemoryMXBean memoryMXBean) {
		...
		return String
			.format("Memory usage stats: [HEAP: %d/%d/%d MB, " + "NON HEAP: %d/%d/%d MB (used/committed/max)]", heapUsed, heapCommitted, heapMax,
				nonHeapUsed, nonHeapCommitted, nonHeapMax);
	}
	...
	public static String getDirectMemoryStatsAsString(BufferPoolMXBean bufferPoolMxBean) {
		if(bufferPoolMxBean == null) {
			...
			return String.format("Direct memory stats: Count: %d, Total Capacity: %d, Used Memory: %d", bufferPoolMxBean.getCount(),
				bufferPoolMxBean.getTotalCapacity(), bufferPoolMxBean.getMemoryUsed());
		}
	}
